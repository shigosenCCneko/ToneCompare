...
public class TfftViewController {
	...
	@FXML ComboBox<String> audioDeviceSelectBox;
	ObservableList<String> audioDeviceOptions;
	List<Integer> audioDeviceNo;
	Class<?> lineClass;
	Mixer.Info[] audioMix;
	int				mixNo;
	...
	final int N_WAVE = 4096;              //サイン波テーブル
	final int LOG2_N_WAVE = 12;           //サイン波テーブルサイズのbit長
	final int FFTviewStep = 4;				//描画時の纏めサイズ
	int SineWaveN_WAVE2[];
...
//	int sqrtTable[][];
	...
	int FFTsampringMag =4;          //音声サンプリング周波数に対するFFTサンプリング倍率
...
//	List <OpeCandi> candidate = new ArrayList<OpeCandi>();
	...
	int stepSize = BasicStepSize;			//RealTimeFFT時の音声読み込みサイズ
	long	fftDrawWaitTime = 100;        //描画スレッド落ち対策のwait
	AudioInput audioin;
	byte[] auBuf = new byte[stepSize * 2];
	byte[] auBuf1 = new byte[stepSize * 2];
	byte[] auBuf2 = new byte[stepSize * 2];

	byte[] readbuf;						//リアルタイムFFT読み出しバッファ
	byte[] writebuf;					//リアルタイムFFT書き込みバッファ
...
//		}
		...
		mixNo = -1;
		//lineClass = TargetDataLine.class;
		AudioFormat fmt = new AudioFormat(44000,16,1,true,false);
		DataLine.Info info = new DataLine.Info(TargetDataLine.class, fmt);


		audioMix = AudioSystem.getMixerInfo();

		audioDeviceOptions = FXCollections.observableArrayList();
		audioDeviceNo = new ArrayList<Integer>();

		for(int i = 0; i < audioMix.length;i++) {
			Mixer mixer = AudioSystem.getMixer( audioMix[i]);
			Line.Info[] line = mixer.getTargetLineInfo();
			if(line.length >0) {
				if(line[0].getClass().toString().contains("Port") == false) {
				//System.out.println(line[0].getClass());
				audioDeviceOptions.add(audioMix[i].getName() );
				audioDeviceNo.add(i);
				}
			}
		...
		audioDeviceSelectBox.setItems(audioDeviceOptions);
	...
	void changeAudioDevice() {
		int i = audioDeviceOptions.indexOf(audioDeviceSelectBox.getValue());

		mixNo = audioDeviceNo.get(i);

		Mixer mixer = AudioSystem.getMixer(audioMix[mixNo]);
		Line.Info[] line = mixer.getTargetLineInfo();
		...
		//	System.out.println(line[j].toString()   );
		}
	...
	private class AudioInput{
		...
		AudioInput(int bit,int channel,int hz){
			try {
				...
				linfo = new DataLine.Info( lineClass,fmt);
				dataline = (TargetDataLine)( AudioSystem.getMixer(audioMix[mixNo]).getLine(linfo));
				}
...
audioin.start();
			...
			drline.setPriority(Thread.MAX_PRIORITY);
	  		drline.start(source);
...
audioin.stop();
  ...
  class DrawFFTLine extends Thread{
		int offset = 4;
		int a,b,c,d,max;
		int cnt = 0;
  ...
  //-------------------- nomal FFT-------------------------------------------
void reDrawFFT() {
	if(tfftButton1.isSelected() && (realTimeFFT.isSelected() == false)) {
		waveFft2(audioRec1,gfft1);
	}else {
		waveFft(audioRec1,gfft1);
	}
...
//
//
////		  String filename = "H:\\fftDataH\\fftdata-alg4-2.txt";
//		  String filename = "H:\\fftDataH\\1243-1357.txt";
//
//
